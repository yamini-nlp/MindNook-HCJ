window.MindNookCrisisBanner = (function () {
  ...
  function renderAcuteBanner(resource, onAcknowledge) {
    ...
    const linkHtml = resource.url ? ` · <a href="${resource.url}" target="_blank" rel="noopener" style="color:#f5c6cc;">${resource.url}</a>` : '';
    const numberHtml = resource.number ? `${resource.number}${linkHtml ? ' · ' : ''}` : '';
    banner.innerHTML = `
      <div style="max-width:760px;margin:0 auto;">
        <div style="font-size:0.95rem;font-weight:500;margin-bottom:8px;">If you are in crisis or thinking about suicide, please reach out for support.</div>
        <div style="font-size:0.85rem;line-height:1.7;margin-bottom:14px;">${resource.name}: ${numberHtml}${resource.url && !numberHtml ? `<a href="${resource.url}" target="_blank" rel="noopener" style="color:#f5c6cc;">${resource.url}</a>` : ''}</div>
        <button id="mindnookAcuteAck" style="padding:9px 20px;border-radius:30px;border:none;background:#f5e6e8;color:#3a1418;font-weight:500;cursor:pointer;">I understand</button>
      </div>`;
    document.body.appendChild(banner);
    document.getElementById('mindnookAcuteAck').onclick = () => {
      banner.remove();
  ...
  function renderPatternCard(containerId, onDismiss) {
    ...
    card.innerHTML = `
      ...
      <button id="mindnookPatternDismiss" style="padding:6px 16px;border-radius:30px;border:1px solid rgba(217,197,178,0.3);background:transparent;color:var(--text2,rgba(240,236,226,0.65));font-size:0.7rem;cursor:pointer;">Dismiss</button>`;
    container.appendChild(card);
    document.getElementById('mindnookPatternDismiss').onclick = () => {
      card.remove();
